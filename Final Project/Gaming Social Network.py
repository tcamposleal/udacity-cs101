...
def create_data_structure(string_input):
    ...
    while string_input.find('.',mark)!=-1:
        user=string_input[mark:string_input.find(' ',mark)]
        network[user]={'friends':[],'games':[]}
        mark=string_input.find('.',mark)+1
    mark=0
    
    while string_input.find('.',mark)!=-1:
        current_string=string_input[mark:string_input.find('.',mark)]
        user=string_input[mark:string_input.find(' ',mark)]
        if current_string.find('is connected to')!=-1:
            connections=current_string[current_string.find('is connected to ')+len('is connected to '):].split(', ')
            for dude in connections:
                network[user]['friends'].append(dude)
        else:
            games=current_string[current_string.find('likes to play ')+len('likes to play '):].split(', ')
            for game in games:
...
# print get_games_liked(network,'Ollie')
...
# print add_connection(network,'Bryant','Debra')
...
# print add_new_user(network,'Tomas',['Tomb Raidder','Sims'])
...
def get_secondary_connections(network, user):
    ...
    for e in get_connections(network,user):
        for i in get_connections(network,e):
...
# print get_secondary_connections(network,'Bryant')
...
# print count_common_connections(network,'Bryant','Ollie')
